                    ...
                    <table class="table border rounded mt-5" >
                        ...
                        <tbody>
                            <?php $sno = 1; while($row = mysqli_fetch_assoc($s_query)){
                                $row['tasks'] = openssl_decrypt($row['tasks'],'aes-256-cbc','supersej123');
                                if($row['id']==$_GET['id'] && md5($row['tasks'])!= $_GET['hashed']){?>
                                    <script>alert("data has been manipulated")</script>
                                <?}
                                ?>
                                <tr>
...
<?php
        ...
        header("location:http://sej-server.dk/todoPhp/index.php?msg=login_first");
    }
?>